...
public partial class tree_conifer : RigidBody3D
{
	...
	public override void _Ready()
	{
		...
		animatedSprite = GetNode<AnimatedSprite3D>("tree_conifer_anim");
		collisionShape = GetNode<CollisionShape3D>("tree_conifer_collide");
		int animationChoice = random.Next(1, 4); 
		
		//Determines speed for this animation so its varied a bit
		int slowdown = random.Next(1, 130);
		animatedSprite.SpeedScale = 1.0f * 0.01f * (float)slowdown;
